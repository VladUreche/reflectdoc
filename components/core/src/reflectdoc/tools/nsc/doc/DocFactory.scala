...
class DocFactory(val host: HostContext, val reporter: Reporter, val settings: doc.Settings) { processor =>
...
//  object compiler extends ScaladocGlobal(settings, reporter)
  ...
  def makeUniverse(): Option[Universe] = {
    ...
    def generate() = {
      ...
      docletInstance match {
        case universer: Universer =>
          val universe = makeUniverse() getOrElse { throw NoCompilerRunException }
